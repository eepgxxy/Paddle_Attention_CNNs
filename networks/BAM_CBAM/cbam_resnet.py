...
class ResNet(nn.Layer):
    def __init__(self, block, layers,  network_type, num_classes, att_type=None):
        ...
        self.fc.weight_attr=paddle.ParamAttr(initializer=nn.initializer.KaimingNormal())
        for key in self.state_dict():
            if key.split('.')[-1]=="weight":
                if "conv" in key:
                    self.state_dict()[key].weight_attr=paddle.ParamAttr(initializer=nn.initializer.KaimingNormal())
                if "bn" in key:
...
if __name__ == '__main__':
    network = ResidualNet("ImageNet", 50, 10, 'BAM')
