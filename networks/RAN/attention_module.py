...
class AttentionModule_pre(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, size1, size2, size3):
        ...
        self.softmax6_blocks = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2D(out_channels, out_channels , kernel_size = 1, stride = 1, bias_attr = False),
            nn.BatchNorm2D(out_channels),
            nn.ReLU(),
            nn.Conv2D(out_channels, out_channels , kernel_size = 1, stride = 1, bias_attr = False),
            nn.Sigmoid()
...
class AttentionModule_stage0(nn.Layer):
...
class AttentionModule_stage1_cifar(nn.Layer):
    # input size is 16*16
...
class AttentionModule_stage2_cifar(nn.Layer):
    # input size is 8*8
...
class AttentionModule_stage3_cifar(nn.Layer):
    # input size is 4*4
    def __init__(self, in_channels, out_channels, size=(8, 8)):
